<template>
  ...
  <swiper
    ...
    class="mySwiper"
  >
    <swiper-slide style="text-align: center" v-for="main_gallare in main_gallares" >
      <div class="main_img_container">
        <img :src="main_gallare.img_url" alt="" loading="lazy" />
        <p class="img_discription">
   {{ main_gallare.img_details }}
        </p>
      </div>
    </swiper-slide>
  </swiper>
          ...
          >
            <swiper-slide v-for="img in topic.imgs">
              <div class="img_container">
                <img :src="img.img_url" alt="" loading="lazy" />
                <p class="img_discription">
                  {{ img.about_img }}
                </p>
              </div>
...
export default {
  ...
  mounted() {
    ...
    fetch("http://localhost:3000/main_gallares")
      .then((res) => res.json())
      .then((data) => (this.main_gallares = data))
      .catch((err) => console.log(err.massage));
  },
  methods: {
    togalle_event(e) {
...
.main_img_container .img_discription {
  display: block;
  position: absolute;
}
...
.img_discription {
  ...
  background-color: var(--main-color); /* Change to match your design */
  border-radius: 5px; /* Change to match your design */
}
</style>
