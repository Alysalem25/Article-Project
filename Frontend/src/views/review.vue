<template>
    <div>
     ...
     <div  class="container"
       ...
       >
         <div>
         <h1 class="text-center">مراجعه المقال</h1>
           <div v-if="article">
             <div class="row1 pt-5">
               <h1 class="full_article_title">{{ article.title }}</h1>
               <p class="mx-3">{{ formattedDate }}</p>  الكاتب:{{ article.author }}
             </div>
             <div class="row">
               <div class="col">
             ...
             <div class="row">
               <p class="full_article" v-html="article.content"></p>
             </div>
             <hr />
             ...
             <div class="row see_note_row" v-if="article.note">
               <div class="see_note">
                 {{ article.note }}
               </div>
             </div>
         ...
         <div class="container3 m-5" v-if="article_have_relations">
           ...
           <div class="row relation_article_row">
             <div
               ...
               v-for="(article_have_relation, index) in article_have_relations"
               :key="index"
             >
               <div class="div_img1">
                 <div>
                   ...
                   <div class="img_detalis">
                     ...
                     <p class="writer">
                       الكاتب:{{ article_have_relation.write_by }}
                     </p>
                   </div>
  ...
  export default {
    ...
    methods: {
      async acceptArticle(status) {
        if (status === "rejected" && !this.note.trim()) {
        ...
        try {
          ...
          if (status === "rejected") {
            requestData.note = this.note; // Save the rejection note
            requestData.status = "rejected";
            requestData.deletedAt = new Date(); // Set the current timestamp to delete in 24 hours
         console.log(requestData)
          }
  
          const response = await axios.post(
            `http://localhost:5000/editarticle/${this.id}`,
            requestData
          );
          ...
          alert(
            `تم ${
              status === "published"
                ? "نشر"
                : "رفض المقال وسيتم حذفه تلقائيًا بعد 24 ساعة"
            } بنجاح!`
          );
          console.log("Article updated:", response.data);
          window.location = "http://localhost:8080/all_articles2";
        } catch (error) {
    ...
    mounted() {
      ...
      // fetch("http://localhost:5000/articles/" + this.id)
      //   .then((res) => res.json())
      //   .then((data) => {
      //     this.article = data;
